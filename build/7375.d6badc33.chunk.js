"use strict";
(self["webpackChunkproject_kru_2023"] = self["webpackChunkproject_kru_2023"] || []).push([[7375],{
...
const FormHead = ({ apiToken, setApiToken, canEditInputs, canRegenerate, isSubmitting }) => {
  ...
  return /* @__PURE__ */ react.createElement(
    HeaderLayout/* HeaderLayout */.T,
    {
      ...
      primaryAction: canEditInputs ? /* @__PURE__ */ react.createElement(Stack/* Stack */.K, { horizontal: true, spacing: 2 }, canRegenerate && apiToken?.id && /* @__PURE__ */ react.createElement(components_Regenerate, { onRegenerate: handleRegenerate, idToRegenerate: apiToken?.id }), /* @__PURE__ */ react.createElement(
        Button/* Button */.z,
        {
...
const ActionBoundRoutes = () => {
  ...
  return /* @__PURE__ */ react.createElement(
    ...
    selectedAction ? /* @__PURE__ */ react.createElement(Stack/* Stack */.K, { spacing: 2 }, routes[actionSection]?.map((route) => {
...
const FormBody = ({
  ...
  return /* @__PURE__ */ react.createElement(ContentLayout/* ContentLayout */.D, null, /* @__PURE__ */ react.createElement(Stack/* Stack */.K, { spacing: 6 }, Boolean(apiToken?.name) && /* @__PURE__ */ react.createElement(ContentBox, { apiToken: apiToken?.accessKey }), /* @__PURE__ */ react.createElement(
...
const ApiTokenCreateView = () => {
    ...
    async () => {
      const [permissions, routes] = await Promise.all(
        ["/admin/content-api/permissions", "/admin/content-api/routes"].map(async (url) => {
          const { data } = await core_utils/* axiosInstance.get */.be.get(url);
          return data.data;
        })
      );
      dispatch({
  ...
  const { status } = (0,lib.useQuery)(
    ...
    async () => {
      const {
        data: { data }
      } = await core_utils/* axiosInstance.get */.be.get(`/admin/api-tokens/${id}`);
      setApiToken({
        ...data
      });
  ...
  const handleSubmit = async (body, actions) => {
    ...
    try {
      const {
        data: { data: response }
      } = isCreating ? await core_utils/* axiosInstance.post */.be.post(`/admin/api-tokens`, {
        ...body,
        lifespan: lifespanVal,
        permissions: body.type === "custom" ? state.selectedActions : null
      }) : await core_utils/* axiosInstance.put */.be.put(`/admin/api-tokens/${id}`, {
        name: body.name,
